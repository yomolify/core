...
class CCXTFeed(with_metaclass(MetaCCXTFeed, DataBase)):
    ...
    def _fetch_ohlcv(self, fromdate=None):
        ...
        while True:
            dlen = len(self._data)
            reply = cli.query(pymkts.Params(f'binancefutures_{self.p.dataname.replace("/", "-")}', self.p.tf, 'OHLCV', start=self.p.fromdate.isoformat())).first().df()
            data = []
            for index, row in reply.iterrows():
                data.append([index.value / 1000000, row['Open'], row['High'], row['Low'], row['Close'], row['Volume']])
            ...
            # All new data has been added, so can break out of this loop
