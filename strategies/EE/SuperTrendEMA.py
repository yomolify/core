...
class SuperTrendEMA(StrategyBase):
    ...
    def __init__(self):
        ...
        for d in self.datas:
            ...
            self.inds[ticker]["sma_slow"] = bt.ind.EMA(d.close,
        ...
        if (self.check_for_live_data and self.status == "LIVE") or not self.check_for_live_data:
            ...
            for i, d in enumerate(available):
                ...
                if current_position > 0:
                    if d.close[0] < self.inds[ticker]["sma_slow"] or d.close[0] < self.inds[ticker]["super_trend"][0]:
                        try:
    ...
    def rebalance_portfolio(self):
        self.rankings = list(filter(lambda d: len(d) > 200, self.altcoins))
