...
class NewYearlyHighsImproved(StrategyBase):
    ...
    def updateStops(self, d):
        ...
        if current_position > 0:
            ...
            if self.pos[ticker]["reset_stop"] is True or self.pos[ticker]["new_sl_price"] and self.pos[ticker]["sl_price"] and self.pos[ticker]["new_sl_price"] > self.pos[ticker]["sl_price"]:
                self.log(f'{ticker} Update stop from {self.pos[ticker]["sl_price"]} to {self.pos[ticker]["new_sl_price"]}')
                ...
                # self.stop_order[ticker] = self.close(data=d, price=self.pos[ticker]["new_sl_price"], exectype=bt.Order.StopTrail, trailpercent=0.2)
                self.stop_order[ticker] = self.sell(data=d, size=current_position/2, price=self.pos[ticker]["new_sl_price"], exectype=bt.Order.StopTrail, trailamount=self.inds[ticker]["average_true_range"][0])
        ...
        for i, d in enumerate(self.rankings[:5]):
            ...
            if current_position:
                try:
                    ...
                    self.log(f'Rebalancing {d._name} to {abs(self.inds[d._name]["roc"][0])}%')
                except Exception as e:
                    self.log("ERROR: {}".format(sys.exc_info()[0]))
                    self.log("{}".format(e))