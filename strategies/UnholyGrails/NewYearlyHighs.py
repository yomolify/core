...
class NewYearlyHighs(StrategyBase):
    ...
    def __init__(self):
        ...
        for d in self.datas:
            self.inds[d] = {}
            self.inds[d]["rolling_high"] = bt.indicators.Highest(d.close, period=self.params.period_rolling_high, plot=True, subplot=False)
            self.inds[d]["rolling_low"] = bt.indicators.Lowest(d.close, period=self.params.period_rolling_low, plot=True, subplot=False)
    ...
    def next(self):
        ...
        for i, d in enumerate(available):
            if abs(self.broker.getposition(d).size > 0.0005):
                if d.close[0] < self.inds[d]['rolling_low'][-1]:
                    self.close(d)
            if abs(self.broker.getposition(d).size < 0.0005):
                if d.close[0] > self.inds[d]['rolling_high'][-1]:
                    self.buy(d)
        #
