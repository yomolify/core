...
# TODO - Make the TPs move with the VWAP
class VWAPMR(StrategyBase):
        ...
        if (self.check_for_live_data and self.status == "LIVE") or not self.check_for_live_data:
            ...
            for i, d in enumerate(available):
                ...
                if current_position > 0:
                    ...
                    #         # self.stop_orders[ticker] = self.close(data=d, price=0.8*d.close[0],
                    #         #                                      exectype=bt.Order.StopTrail,
                    #         #                                      trailamount=d.close[0] / 10)
                    #         self.stop_orders[ticker] = self.close(data=d, price=self.pos[ticker]["sl_price"],
                    ...
                    #         self.cancel(self.stop_orders[ticker])
                    #     self.stop_orders[ticker] = None
                    #     self.stop_orders[ticker] = self.close(data=d, price=self.pos[ticker]["new_sl_price"], exectype=bt.Order.Stop)
                    #     self.pos[ticker]["reset_stop"] = False
                        # self.stop_orders[ticker] = self.close(data=d, price=self.pos[ticker]["new_sl_price"], exectype=bt.Order.StopTrail, trailpercent=0.1)
                        # self.stop_orders[ticker] = self.sell(data=d, size=current_position/2, price=self.pos[ticker]["new_sl_price"], exectype=bt.Order.Stop)
                # self.log(self.pos[ticker]["size"])
                # self.log(self.tp_orders[ticker])
                # self.log(self.stop_orders[ticker])
                if current_position > 0.0001:
                    move_stop_to_be = False
                    if self.entry_type[ticker] == "Long":
                        if len(self.tp_orders[ticker]) == 0:
                            self.tp_orders[ticker].append(self.sell(data=d, size=self.pos[ticker]["size"] / 3, price=self.inds[ticker]["vwap"][0], exectype=bt.Order.Limit))
                            self.tp_orders[ticker].append(self.sell(data=d, size=self.pos[ticker]["size"] / 3, price=self.inds[ticker]["vwap"][0] + 0.5 * self.inds[ticker]["atr"][0], exectype=bt.Order.Limit))
                            self.tp_orders[ticker].append(self.sell(data=d, size=self.pos[ticker]["size"] / 3, price=self.inds[ticker]["vwap"][0] + 1 * self.inds[ticker]["atr"][0], exectype=bt.Order.Limit))
                        ...
                        if move_stop_to_be:
                            for stop_order in self.stop_orders[ticker]:
                                self.cancel(stop_order)
                                self.stop_orders[ticker].remove(stop_order)
                            self.log((self.pos[ticker]["price"] + d.close[0]) / 2)
                            self.stop_orders[ticker].append(self.close(data=d, price=(self.pos[ticker]["price"] + d.close[0]) / 2, exectype=bt.Order.Stop))
                        # When away from vwap and consolidating, enter with tp at vwap
                elif current_position < -0.0001:
                    move_stop_to_be = False
                    if self.entry_type[ticker] == "Short":
                        if len(self.tp_orders[ticker]) == 0:
                            self.tp_orders[ticker].append(self.buy(data=d, size=self.pos[ticker]["size"]/3, price=self.inds[ticker]["vwap"][0], exectype=bt.Order.Limit))
                            self.tp_orders[ticker].append(self.buy(data=d, size=self.pos[ticker]["size"]/3, price=self.inds[ticker]["vwap"][0] - 0.5*self.inds[ticker]["atr"][0], exectype=bt.Order.Limit))
                            self.tp_orders[ticker].append(self.buy(data=d, size=self.pos[ticker]["size"]/3, price=self.inds[ticker]["vwap"][0] - 1*self.inds[ticker]["atr"][0], exectype=bt.Order.Limit))
                        elif len(self.tp_orders[ticker]) <= 3:
                            for tp_order in self.tp_orders[ticker]:
                                if tp_order.status == 4:
                                    move_stop_to_be = True
                ...
                if abs(current_position) < 0.0001:
                    ...
                    if self.tp_orders[ticker]:
                        for tp_order in self.tp_orders[ticker]:
                            self.cancel(tp_order)
                        self.tp_orders[ticker] = []
                    if self.inds[ticker]["adx"][0] < 20 and d.close[0] < self.inds[ticker]["vwap"][0] - 2*self.inds[ticker]['atr'][0] and (self.inds[ticker]["rsi"][0] < 45 or self.inds[ticker]["rsi"][-1] < 45 or self.inds[ticker]["rsi"][-2] < 45):
                        self.entry_type[ticker] = "Long"
                        try:
                    ...
                    elif self.inds[ticker]["adx"][0] < 20 and d.close[0] > self.inds[ticker]["vwap"][0] + 2*self.inds[ticker]['atr'][0]:
