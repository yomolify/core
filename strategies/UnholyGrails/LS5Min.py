...
class LS5Min(StrategyBase):
    ...
    params = (
        ...
        ('period_highest_vol_sma', 50),
        ('period_lowest_vol_sma', 100),
    )
    ...
    def __init__(self):
        ...
        for d in self.datas:
            if '1H' in d._name:
                ticker = d._name
                self.inds[ticker] = {}
                self.inds[ticker]["1H_sma"] = bt.ind.HMA(d, period=self.params.period_sma_fast, plot=True, subplot=False)
        for d in self.datas:
            ticker = d._name
            self.pos[ticker] = {}
            self.inds[ticker] = {}
            ...
            # self.inds[ticker]["highest_vol_sma"] = bt.ind.Highest(self.inds[ticker]["vol_sma"],
            #     period=self.params.period_highest_vol_sma, plot=False, subplot=False)
            #
            # self.inds[ticker]["lowest_vol_sma"] = bt.ind.Lowest(self.inds[ticker]["vol_sma"],
            #                                       period=self.params.period_lowest_vol_sma, plot=False, subplot=False)
            # self.inds[ticker]["diff_vol_sma"] = bt.ind.NonZeroDifference(self.inds[ticker]["highest_vol_sma"], self.inds[ticker]["lowest_vol_sma"], plot=True, subplot=True)
            ...
            # self.inds[ticker]["roc_vol_sma"] = bt.ind.ROC(self.inds[ticker]["vol_sma"], plot=True, subplot=True)
            ...
            for i, d in enumerate(available):
                ...
                if '1H' not in d._name:
                    ...
                    if current_position > 0:
                        ...
                        # if adx is making higher highs and then reverses, that is reversal. if above sma 200 then short, if below then long
                        if self.inds[ticker]['rsi'][0] > 80 and self.pos[ticker]["profit_percentage"] > 0:
                            try:
                                ...
                                self.log("{}".format(e))
                        # if d.close[0] < self.inds['1H_ETH-USDT']['sma_veryslow'][0]:
                        #     try:
                        #         order = self.add_order(data=d, target=0, type='market')
                        #         if ticker in self.stop_order:
                        #             self.cancel(self.stop_order[ticker])
                        ...
                        #         self.log("ERROR: {}".format(sys.exc_info()[0]))
                        #         self.log("{}".format(e))
                        if self.pos[ticker]["profit_percentage"] < -2:
                            try:
                                ...
                                self.log("ERROR: {}".format(sys.exc_info()[0]))
                                self.log("{}".format(e))
                        #    Decreasing volume while in long
                        # if self.inds[ticker]["sma_slow"][0] < self.inds[ticker]["sma_veryslow"][0]:
                        #     try:
                        #         order = self.add_order(data=d, target=0, type='market')
                        #         if ticker in self.stop_order:
                        #             self.cancel(self.stop_order[ticker])
                        #         self.log('Decreasing VOL Sell Signal')
                        #         print(self.pos[ticker]["profit_percentage"])
                        #     except Exception as e:
                        #         self.log("ERROR: {}".format(sys.exc_info()[0]))
                        #         self.log("{}".format(e))
                    ...
                    if current_position == 0:
                        ...
                        if self.inds[ticker]["roc_sma_slow"][0] < 0 and self.inds[ticker]["roc_sma_slow"][0] > self.inds[ticker]["roc_sma_slow"][-1] and d.volume[0] > self.inds[ticker]['vol_sma_slow'][0] or self.inds[ticker]["rsi"][0] < 20:
                        # if d.volume[0] > self.inds[ticker]['vol_sma'][0] and d.close[0] > self.inds[ticker]['sma_slow'][0] - 2*self.inds[ticker]["average_true_range"][0] and self.inds[ticker]["rsi"] < 60:
                            try:
                                ...
                                self.log("ERROR: {}".format(sys.exc_info()[0]))
                                self.log("{}".format(e))
                        # elif d.close[0] < self.inds[ticker]["sma_veryslow"][0]:
                        #     try:
                        #         self.add_order(data=d, target=-(
                        #                 (self.p.order_target_percent / 100) * volatility_factor),
                        #                        type="market")
