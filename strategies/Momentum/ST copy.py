...
class STCOPY(StrategyBase):
    ...
    def __init__(self):
        ...
        for d in self.datas_5m:
            ...
            self.inds[ticker]["pct_change_vol_sma"] = bt.indicators.PctChange(self.inds[ticker]["volsma_fast_5m"], plot=False, subplot=False)
    ...
    def next(self):
        if (self.check_for_live_data and self.status == "LIVE") or not self.check_for_live_data:
            ...
            # self.datas_5m.sort(reverse=True, key=lambda d: abs(self.inds[d._name]["pct_change_vol_sma"][0] * self.inds[d._name]["pct_change"][0]))
            for d in self.datas_5m:
                ticker = d._name
                # ticker = ticker[3:]
