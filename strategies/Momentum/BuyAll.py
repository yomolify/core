...
class Momo(StrategyBase):
    ...
    def __init__(self):
        ...
        for d in self.datas:
            ...
            self.flags[ticker]["to_close"] = False
            self.flags[ticker]["to_open"] = False
    ...
    def next(self):
        ...
        for d in self.datas:
            if self.flags[d._name]["to_close"]:
                self.close(d)
                self.flags[d._name]["to_close"] = False
        for d in self.datas:
            if self.flags[d._name]["to_open"]:
                value = self.broker.getvalue()
                # self.buy(d, size=((value/self.p.base_pos_cash)/d.close[0])*self.inds[d._name]["volatility"][0]) \
                self.add_order(d, target=(self.p.order_target_percent / 100) * self.inds[d._name]["volatility"][0])
                self.flags[d._name]["to_open"] = False
            ...
            for d in self.datas:
                ...
                if True or self.mean_returns[-1] > 0:
                    ...
                    if self.returns[ticker][-1] > self.mean_returns[-1]:
                        ...
                        if current_position == 0 and cash > value/self.p.base_pos_cash:
                            ...
                            self.log(f'enter long {ticker}')
                            self.entry_order[ticker] = self.add_order(d, target=(self.p.order_target_percent / 100) * self.inds[ticker]["volatility"][0])
                            # self.entry_order[ticker] = self.buy(d, size=((value/self.p.base_pos_cash)/d.close[0])*self.inds[ticker]["volatility"][0])
    ...
    def rebalance_portfolio(self):
        ...
        for i, coin in enumerate(no_position_coins):
            self.log(f'No position: {coin._name}')
        if len(no_position_coins):
            ...
            for i, coin in enumerate(in_position_coins):
                self.log(f'In position: {coin._name}')

            for i, pos_coin in enumerate(in_position_coins):
                if i < len(no_position_coins) and self.inds[(no_position_coins[i])._name]["pct_change"][0] > 0:
                    if self.inds[pos_coin._name]["pct_change"][0] < self.inds[(no_position_coins[i])._name]["pct_change"][0]:
                        self.log(f'Close {pos_coin._name} {self.inds[pos_coin._name]["pct_change"][0]} & Open {(no_position_coins[i])._name} {self.inds[(no_position_coins[i])._name]["pct_change"][0]}')
                        current_position = self.get_position(d=pos_coin, attribute='size')
                        # self.close(d=coin)
                        self.log(f'{pos_coin._name} {current_position}')
                        # self.sell(d=pos_coin)
                        self.flags[pos_coin._name]["to_close"] = True
                        self.flags[(no_position_coins[i])._name]["to_open"] = True
                        # self.sell(d=coin, size=current_position/2)
                        # self.log(self.pos[pos_coin._name]["profit_arr"][-1])
                        # self.add_order(data=coin, target=0, type='market')
                        # self.add_order(no_position_coins[i], target=(self.p.order_target_percent / 100) * self.inds[(no_position_coins[i])._name]["volatility"][0])
                # break
