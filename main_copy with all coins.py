...
if __name__ == '__main__':
    ...
    if ENV == PRODUCTION:  # Live trading with Binance
        ...
        # Altcoin Universe
            ...
            #     )
            #
            #     cerebro.adddata(data, name=f'1m_{ticker}')
                # cerebro.resampledata(data,
                #                                           timeframe=bt.TimeFrame.Minutes,
                #                                           compression=5, name=f'5m_{ticker}')
            #
            for ticker in tickers:
                data = bt.feeds.MarketStore(
                    symbol=f'binancefutures_{ticker}',
        ...
        #     strategy,
        #     pct_change_period=range(50, 100))
        ...
        if args.plot == True:
            p = BacktraderPlotting(style='candle', scheme=Blackly(), volume=False)
            cerebro.plot(p)
